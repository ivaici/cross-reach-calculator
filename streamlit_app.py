...
# --- Header (logo aligned with title) ---
def header_with_logo(img_path: str, title: str, img_px: int = 40):
    with open(img_path, "rb") as f:
        data_uri = f"data:image/png;base64,{b64encode(f.read()).decode()}"
    st.markdown(
        f"""
        <div style="display:flex; align-items:center; gap:12px; margin: 0 0 8px 0;">
            <img src="{data_uri}" style="height:{img_px}px; border-radius:8px;" />
...
header_with_logo("mindshare_lithuania_logo.jpg", "Cross-Reach Calculator", img_px=40)
...
# =====================================================================
    ...
    R_raw = {ch: pct_to_unit_local(marg_edit.loc[i, "Reach %"]) for i, ch in enumerate(chosen)}
    ...
    if any(not (0 <= R_raw[ch] <= 1) for ch in chosen):
        st.error("Media reach values must be between 0 and 100%."); st.stop()
    ...
    for a in chosen:
        ...
        if ua is None or not (0 <= ua <= 1):
            st.error("Monthly usage U(A) (diagonal) must be 0–100%."); st.stop()
        if R[a] - ua > 1e-9:
            st.error(f"{'Attentive ' if use_attentive else ''}reach for {a} ({R[a]:.2%}) "
                     f"cannot exceed monthly usage U({a}) ({ua:.2%})."); st.stop()
    ...
    for i, a in enumerate(chosen):
        for j, b in enumerate(chosen):
            ...
            uab = U.loc[a, b]
            if uab is None:
                st.error(f"Please fill monthly usage overlap U({a}∩{b}) in the matrix below."); st.stop()
    ...
    for a in chosen: P2.loc[a, a] = R[a]
    for i, a in enumerate(chosen):
        for j, b in enumerate(chosen):
            if j <= i: continue
            uab = U.loc[a, b]
